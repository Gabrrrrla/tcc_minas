...
def _revert_prb(params: dict) -> dict:
    ...
    # TODO: persist RAN allocations (no table yet) so this can restore exact values.
    print(f"[ran-nssmf] revert_prb intent={intent_id}")
    return {"status": "reverted", "intent_id": intent_id, "note": "allocation restored to pre-intent state"}
